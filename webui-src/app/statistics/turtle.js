...
const Turtle = {
  ...
  async load() {
    ...
    try {
      ...
      // Check cumulative traffic from Core services 0x0014 (Turtle) & 0x0015 (Tunnel)
  ...
  view() {
    ...
    // Aggregate repartition of tunnel requests by peer & depth (as in Qt GUI TRHistogram)
    ...
    return m('.turtle-view', [
      Turtle.error && m('.statistics-error', [m('i.fas.fa-exclamation-triangle'), Turtle.error]),

      // ── Top Summary KPI Cards (Speed + Cumulative Volume in KB/MB/GB) ──
      traffic && m('.bandwidth-summary-grid', [
        m('.bandwidth-stat-card', [
          m('.bandwidth-stat-card__icon.bandwidth-stat-card__icon--queue', m('i.fas.fa-random')),
        ...
        m('.bandwidth-stat-card', [
          m('.bandwidth-stat-card__icon.bandwidth-stat-card__icon--out', m('i.fas.fa-route')),
          m('.bandwidth-stat-card__body', [
        ...
        m('.bandwidth-stat-card', [
          m('.bandwidth-stat-card__icon.bandwidth-stat-card__icon--session', m('i.fas.fa-shield-alt')),
          m('.bandwidth-stat-card__body', [
      ...
      m('.turtle-panel.turtle-data-panel', [
        ...
        m('.turtle-controls-bar', [
          m('.turtle-subtabs', [
            m('button.turtle-subtab[type=button]', {
              class: Turtle.activeTab === 'tunnels' ? 'active' : '',
              onclick: () => { Turtle.activeTab = 'tunnels'; },
            }, [
              m('i.fas.fa-network-wired'),
              m('span', 'Active Tunnels'),
              m('span.turtle-badge', Turtle.tunnelsInfo.length),
            ]),
            m('button.turtle-subtab[type=button]', {
              class: Turtle.activeTab === 'repartition' ? 'active' : '',
              onclick: () => { Turtle.activeTab = 'repartition'; },
            }, [
              m('i.fas.fa-chart-bar'),
              m('span', 'TR Repartition'),
              m('span.turtle-badge', totalTRs),
            ]),
            m('button.turtle-subtab[type=button]', {
              class: Turtle.activeTab === 'searches' ? 'active' : '',
              onclick: () => { Turtle.activeTab = 'searches'; },
            }, [
              m('i.fas.fa-search'),
              m('span', 'Search Requests'),
              m('span.turtle-badge', Turtle.searchReqsInfo.length),
            ]),
            m('button.turtle-subtab[type=button]', {
              class: Turtle.activeTab === 'requests' ? 'active' : '',
              onclick: () => { Turtle.activeTab = 'requests'; },
            }, [
              m('i.fas.fa-paper-plane'),
              m('span', 'Tunnel Requests'),
              m('span.turtle-badge', Turtle.tunnelReqsInfo.length),
            ]),
          ]),

          m('.turtle-filter-wrap', [
            m('i.fas.fa-search.turtle-filter-icon'),
            m('input.turtle-filter-input[type=text]', {
              placeholder: 'Filter by ID, peer or hash…',
              value: Turtle.filterQuery,
              oninput: (e) => { Turtle.filterQuery = e.target.value; },
            }),
            Turtle.filterQuery && m('button.turtle-filter-clear[type=button]', {
              onclick: () => { Turtle.filterQuery = ''; },
            }, m('i.fas.fa-times')),
          ]),
        ]),
        ...
        Turtle.activeTab === 'tunnels' && Turtle.hashesInfo.length > 0 && m('.turtle-hash-bar', [
          ...
          m('.turtle-hash-chips', [
            m('button.turtle-chip[type=button]', {
              class: Turtle.selectedHash === '' ? 'active' : '',
            ...
            Turtle.hashesInfo.map((h) => {
              ...
              return m('button.turtle-chip[type=button]', {
                class: Turtle.selectedHash === hashStr ? 'active' : '',
                onclick: () => {
        ...
        // ── Tab 2: Tunnel Requests Repartition (As seen in Qt GUI TRHistogram) ──
          ...
          totalTRs > 0
            ? m('.turtle-repartition-view', [
              m('.turtle-repartition-grid', [
                ...
                // Repartition by Depth (Hops 0 to 7)
                m('.turtle-repartition-card', [
                  m('h4.turtle-repartition-title', [
